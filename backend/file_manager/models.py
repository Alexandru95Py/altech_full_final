...
class FileDownloadLog(models.Model):
    """
    Model pentru înregistrarea descărcărilor de fișiere.
    """
    ...
    def __str__(self):
        return f"{self.user.username} downloaded {self.file.name} at {self.downloaded_at}"
...
class ProtectedDocument(models.Model):
    ...
    """
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='protected_docs_file_manager')
